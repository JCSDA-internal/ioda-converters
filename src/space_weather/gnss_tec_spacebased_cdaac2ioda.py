...
def get_obs_data(ifile, get_obs_data_args):
    ...
    obs_data[("xECEFPositionLEO", "MetaData")] = np.array(ds['x_LEO'][:])
    obs_data[("yECEFPositionLEO", "MetaData")] = np.array(ds['y_LEO'][:])
    obs_data[("zECEFPositionLEO", "MetaData")] = np.array(ds['z_LEO'][:])
