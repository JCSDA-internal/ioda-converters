...
!         type(obs_conv_pt),pointer,intent(in) :: obslist
         ...
         if( datatype==120 .or.  datatype==187 ) then
            this%nvars=5
             ...
             do k=1,numfield
               ...
               if(k==1 .or. k==2 .or. k==3 .or. k==11) then
                 ...
                 if( trim(cgroupname(k))=="@ObsValue" ) then
                    ...
                    if(trim(this%variable_names(n))=="surface_pressure") rs1d=rs1d*100.0
                 endif
...
!
       ...
       subroutine getdatar1d(obslist,rs1d,numobs,vname,ivar)
         ...
         real(r_single),intent(inout)   :: rs1d(:)
         ...
         do while(associated(thisobs))
            ...
            elseif(vname=='error' .and. thisobs%iferror) then
               ...
               enddo
            elseif(vname=='preqc' .and. thisobs%ifquality) then
               do k=1,thisobs%numlvl
                  rs1d(i+k-1)=thisobs%quality((k-1)*numvar+ivar)
               enddo
            endif
