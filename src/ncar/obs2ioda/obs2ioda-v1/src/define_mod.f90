...
subroutine set_brit_obserr_hyper(name_inst, ichan, obserr)

! set brightness temperature observation errors of hyper-spectral sensors

! For now it is a temporary subroutine to assign IASI observation errors based on
! https://github.com/comgsi/fix/blob/master/global_satinfo.txt

   implicit none

   character(len=*), intent(in)  :: name_inst  ! instrument name eg. iasi_metop-a
   integer(i_kind),  intent(in)  :: ichan      ! channel index
   real(r_kind),     intent(out) :: obserr     ! obserr of ichan
...
end subroutine set_brit_obserr_hyper
