...
# define whether to use Machine Learning model winds instead of GMF
...
class GnssrL2(object):
    ...
    def read_gnssr_files(self):
        ...
        for root, dirs, files in os.walk(self.gnssr_dir, topdown=True):       
            self.gnssrData["file_list"] += [os.path.join(root, filename) for filename in files 
                                            if "spire_gnss-r_L2_NOCCygOcnm" in filename]
        ...
        self.gnssrData["quality_flags_2"] = np.zeros(nlocs)
        self.gnssrData["channel"] = np.zeros(nlocs)
        self.gnssrData["ddm_ant"] = np.zeros(nlocs)
        self.gnssrData["mss"] = np.zeros(nlocs)
        self.gnssrData["rx_id"] = np.zeros(nlocs)
        self.gnssrData["tx_svn"] = np.zeros(nlocs)
        self.gnssrData["noc_quality_flag"] = np.zeros(nlocs)
        self.gnssrData["noc_quality_flag_ddm"] = np.zeros(nlocs)
        ...
        for filename in self.gnssrData["file_list"]:
            ...
            # Format times
            timestamp_utc = dataset_input['ddm_time'][:]  # 1D [time]  seconds since start of the day
            time_deltavals = np.array([timedelta(seconds=i) for i in timestamp_utc])
            file_start_timestring = nc_attrs['file_start_time'][0:10]  # only want the day and not the time
            file_start_datetime = datetime.strptime(file_start_timestring, '%Y-%m-%d')  # YYYY-MM-DD 00:00:00
            ...
            # Which model to use: machine learning or GMF
            if (UseMachineLearningModel):
                self.gnssrData["sample_flags"][ns:ns+nlocs_local] = np.array(dataset_input['wind_confidence_ml'][:])
                self.gnssrData["wind_speed"][ns:ns+nlocs_local] = np.array(dataset_input['wind_ml'][:])
                self.gnssrData["wind_speed_error"][ns:ns+nlocs_local] = np.array(dataset_input['wind_std_ml'][:])
            else:
            ...
            self.gnssrData["incidence_angle"][ns:ns+nlocs_local] = np.array(dataset_input['nasa_sp_inc_angle'][:])
            self.gnssrData["quality_flags"][ns:ns+nlocs_local] = np.array(dataset_input['nasa_quality_flags'][:])
            self.gnssrData["quality_flags_2"][ns:ns+nlocs_local] = np.array(dataset_input['nasa_quality_flags_2'][:])
            self.gnssrData["noc_quality_flag"][ns:ns+nlocs_local] = np.array(dataset_input['noc_quality_flag'][:])
            self.gnssrData["noc_quality_flag_ddm"][ns:ns+nlocs_local] = np.array(dataset_input['noc_quality_flag_ddm'][:])
    ...
    def setup_ioda_vars(self):
        ...
        self.varAttrs[('specularPointIncidenceAngle', 'MetaData')]['_FillValue'] = np.nan  # avoid outputting Nan
        self.outdata[('qualityFlags', 'MetaData')] = self.gnssrData["quality_flags"][loc_idxs].astype('int32')
        self.outdata[('qualityFlags2', 'MetaData')] = self.gnssrData["quality_flags_2"][loc_idxs].astype('int32')
        self.outdata[('meanSquareSlope', 'MetaData')] = self.gnssrData["mss"][loc_idxs].astype('float32')
        self.varAttrs[('meanSquareSlope', 'MetaData')]['_FillValue'] = np.nan  # avoid outputting Nan
