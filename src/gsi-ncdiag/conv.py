        ...
        for v in self.obsvars:
            for p in platforms:
                ...
                for var in self.df.variables.values():
                    vname = var.name
                    if (vname in cd.geovals_metadata_dict.keys()) or (
                            vname in cd.geovals_vars.keys()):
                        vdata = var[...].data
                        ...
                        if vname in cd.geovals_metadata_dict.keys():
                            ...
                            var_out = ncout.createVariable(
                                cd.geovals_metadata_dict[vname], vdata.dtype, dims)
                            var_out[...] = vdata[idx, ...]
                ...
                for o in range(len(outvars)):
                    ...
                    for key, value in gsivars.items():
                        ...
                        if "Errinv" in key:
                            ...
                            if "u_Obs" in key:
                                gsimeta[key] = self.df['u_Observation'][idx] - \
                                    self.df[key][idx]
                            elif "v_Obs" in key:
                                gsimeta[key] = self.df['v_Observation'][idx] - \
                                    self.df[key][idx]
                            else:
                                gsimeta[key] = self.df['Observation'][idx] - \
                                    self.df[key][idx]
                        else:
                    ...
                    for i in range(len(obsdata)):
                        # observation data
                        outdata[recKey][locKeys[i]][varDict[outvars[o]]
                                                    ['valKey']] = obsdata[i]
                        # observation error
                        outdata[recKey][locKeys[i]][varDict[outvars[o]]
                                                    ['errKey']] = obserr[i]
                        ...
                        outdata[recKey][locKeys[i]
                                        ][varDict[outvars[o]]['qcKey']] = obsqc[i]
