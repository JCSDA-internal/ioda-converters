...
if __name__ == '__main__':
    ...
    outdata[('aaodL2QcWithoutAod440Le0.4Threshold', 'MetaData')] = np.where(f3['if_retrieval_is_l2(without_l2_0.4_aod_440_threshold)'] == 1, 0, 1)

    # Whether Coincident_AOD440nm in aeronet_cad.txt reaches Level 2.0 (0: yes, 1: no)
    outdata[('aodL2Qc', 'MetaData')] = np.where(f3['if_aod_is_l2'] == 1, 0, 1)

    # aaod inversion type: 0 for ALM20 and 1 for ALM15
    outdata[('aaodL2Qc', 'MetaData')] = np.where(f3['inversion_data_quality_level'] == 'lev20', 0, 1)
    ...
    outdata[('sensorChannelNumber', 'MetaData')] = np.int32(aeronetinv_chan)

    # Add global atrributes
    DimDict['Location'] = nlocs
    DimDict['Channel'] = aeronetinv_chan
    AttrData['ioda_object_type'] = 'AAOD'
