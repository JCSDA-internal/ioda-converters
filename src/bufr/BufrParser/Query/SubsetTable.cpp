...
namespace bufr {
    ...
    {
        BufrNode root;
        root.mnemonic = dataProvider_.getTag(dataProvider_.getInode());
        root.type = NodeType::Subset;
        root.nodeIdx = dataProvider_.getInode();

        // Parse the entire bufr tree using this recursive function to build the tree
        // of BUFRNodes.
        processNode(root);
...
//        }
    ...
    void SubsetTable::processNode(BufrNode& parent)
    {
        auto nodeIdx = parent.nodeIdx + 1;

        auto lastNode = dataProvider_.getLink(parent.nodeIdx) - 1;
        if (lastNode == -1) lastNode = dataProvider_.getIsc(dataProvider_.getInode());

        std::cout << " << " << parent.nodeIdx  << " " << dataProvider_.getTag(lastNode) << std::endl;
        ...
        while (nodeIdx != 0 && nodeIdx <= lastNode)
        {
            ...
            auto typ = dataProvider_.getTyp(nodeIdx);
            if (typ == Typ::DelayedRep ||
                typ == Typ::DelayedRepStacked ||
                typ == Typ::DelayedBinary ||
                typ == Typ::FixedRep ||
                typ == Typ::Repeat) // Any Replication
            {
                newNode.type = NodeType::Replication;
                processNode(newNode);
            }
            else if (typ == Typ::Sequence) // A Sequence
            {
                newNode.type = NodeType::Sequence;
                processNode(newNode);
            }
            else if (typ == Typ::Number) // Value nodes
            {
                newNode.type = NodeType::Scalar;
            }
            else if (typ == Typ::Character)
            {
                newNode.type = NodeType::String;
            }
