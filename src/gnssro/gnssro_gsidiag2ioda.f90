...
program gnssro_gsidiag_full2small
   ...
   type gnssro_type
      ...
      real(real32), allocatable  :: lat(:)
      real(real32), allocatable  :: lon(:)
      real(real32), allocatable  :: time(:)
      integer(int64), allocatable :: epochtime(:)
      real(real32), allocatable  :: geoid(:)
      real(real32), allocatable  :: rfict(:)
      real(real32), allocatable  :: azim(:)
      real(real32), allocatable  :: impact_height(:)
      real(real32), allocatable  :: impact_parameter(:)
      real(real32), allocatable  :: bnd_obs(:)
      integer, allocatable  :: bnd_effqc(:)
      real(real32), allocatable  :: bnd_obserr(:)
      real(real32), allocatable  :: bnd_gsihofx(:)
      real(real32), allocatable  :: bnd_obserr_final(:)
      real(real32), allocatable  :: bnd_obserr_adjust(:)
      real(real32), allocatable  :: surface_geopotential_height(:)
      real(real32), allocatable  :: air_temperature(:, :)
      real(real32), allocatable  :: virtual_temperature(:, :)
      real(real32), allocatable  :: specific_humidity(:, :)
      real(real32), allocatable  :: geopotential_height(:, :)
      real(real32), allocatable  :: air_pressure(:, :)
      real(real32), allocatable  :: geopotential_height_levels(:, :)
      real(real32), allocatable  :: air_pressure_levels(:, :)
...
! optional 3rd argument: "1" means geoval will be written
   ...
   if (geovalwrite == "1") then
      ...
      call check(NF90_GET_VAR(ncid_in, varid_geoval_nlevs1, gnssro_data%air_pressure_levels, start=(/1, 1/), &
      	& count=(/nlevs1, nobs/)))
...
! - since January 1, 1970
   SUBROUTINE toepochtime(ANATIME, TIMEOFFSET, EPOCHTIME)
      IMPLICIT NONE
      character(len=10) ANATIME
      REAL(real32) :: TIMEOFFSET  !unit: hour
      INTEGER :: IDATE(4)
      INTEGER :: NMIN
